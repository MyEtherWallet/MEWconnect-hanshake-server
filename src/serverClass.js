...
export default class SignalServer {
  ...
  receiverIncomming(socket, details) {
    try {
      ...
      if (this.invalidHex(details.connId))
                  ...
                  this.redis
                    ...
                    .then(_result => {
                      if (_result) {
                        ...
                        socket
                          .to(details.connId)
                          .emit(signal.confirmationFailedBusy)
                      }
                    })
                    .catch(error => {
                      errorLogger.error(
  ...
  ioConnection(socket) {
    try {
      ...
      if (this.invalidHex(token.connId))
        throw new Error('Connection attempted to pass an invalid connection ID')
      switch (connector) {
      ...
      socket.on(signal.tryTurn, connData => {
        ...
        this.redis
          .locateMatchingConnection(connData.connId)
          .then(_result => {
            if (_result) {
              ...
              try {
                ...
                this.createTurnConnection()
                  .then(_results => {
                    ...
                    this.io
                      .to(connData.connId)
                      .emit(signal.turnToken, { data: _results.iceServers })
                    turnLog(
                      `ice servers returned. token.iceServers: ${
                        _results.iceServers
                      }`
                    )
                  })
              ...
              errorLogger.error(
                ' FAILED TO LOCATE MATCHING CONNECTION FOR TURN CONNECTION ATTEMPT'
              )
              turnLog(
                `Failed to locate matching connection for TURN connection attempt with connection ID ${
                  connData.connId
                }`
              )
            }
          ...
          .catch(_error => {
            ...
            turnLog(
              `locateMatchingConnection threw an error looking for connection ID: ${
                connData.connId
              }`
            )
          })
